...
/* ─── Mini charts ─── */
const MiniLine = () => (
  <Svg width="55" height="32" viewBox="0 0 65 40">
    ...
    <View style={s.root}>
          ...
          return (
            <TouchableOpacity key={i} onPress={() => setMeteo(w.label)} activeOpacity={0.7}>
              <View style={[s.meteo, { width: wSize, height: wSize, borderRadius: wSize / 2 }, sel && s.meteoOn]}>
                <MaterialCommunityIcons name={w.icon as any} size={compact ? 20 : 22} color={sel ? '#FFF' : '#2A4A5A'} />
              </View>
            </TouchableOpacity>
          );
        })}
      ...
      <View style={s.grid}>
        ...
        <View style={s.gridRow}>
          ...
          <TouchableOpacity style={[s.card, { backgroundColor: '#1E7F85' }]} activeOpacity={0.7} onPress={() => Alert.alert('Buongiorno!', 'Connessione AI in arrivo...')}>
...
const s = StyleSheet.create({
  ...
  cardLbl: { fontSize: 9, fontWeight: '600', color: '#4A4A40' },
  cardBold: { fontSize: 11, fontWeight: '800', color: '#1A3535' },
  cardInp: {
    fontSize: 12,
    fontWeight: '700',
    color: '#1A3535',
  ...
  cardVal: { fontSize: 11, fontWeight: '700', color: '#1A3535' },
  cardValBold: { fontSize: 12, fontWeight: '800' },
  ...
  storicoT: { fontSize: 12, fontWeight: '800', color: '#1A3535' },
  storicoDay: { fontSize: 8, fontWeight: '600', color: '#5A7575' },
  storicoVal: { fontSize: 14, fontWeight: '900', color: '#1A3535' },
  storicoSub: { fontSize: 7, fontWeight: '600', color: '#7A9090', marginTop: 1 },

  /* 7. Filters + Salva */
  filterRow: { flexDirection: 'row', gap: 6, marginBottom: 5 },
  filterBtn: {
    flex: 1,
    backgroundColor: '#E0DBC8',
    borderRadius: 10,
